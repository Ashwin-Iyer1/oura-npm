...
export var OuraDashboard = function (_a) {
    var accessToken = _a.accessToken, _b = _a.startDate, initialStartDate = _b === void 0 ? new Date(Date.now() - 30 * 24 * 60 * 60 * 1000).toISOString().split('T')[0] : _b, _c = _a.endDate, initialEndDate = _c === void 0 ? new Date().toISOString().split('T')[0] : _c;
    var _d = useState(initialStartDate), startDate = _d[0], setStartDate = _d[1];
    var _e = useState(initialEndDate), endDate = _e[0], setEndDate = _e[1];
    var _f = useOuraData({ accessToken: accessToken, startDate: startDate, endDate: endDate }), data = _f.data, loading = _f.loading, error = _f.error;
    if (loading)
